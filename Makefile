...
clean-cli:
	cd spawnctl/ && \
	./spawnctl maintenance uninstall
...
build-cli:
	...
	MIX_ENV=prod mix release spawnctl
...
build-proxy-image:
	# When we migrate to a new version of buildx, we can do:
	# docker buildx build -f Dockerfile-proxy --tag ${proxy-image} --attest type=provenance,mode=max .
...
build-elixir-sdk-image:
	docker build --no-cache \
		-f Dockerfile-elixir-example \
		-t ${spawn-sdk-example-image} .

build-activator-simple-image:
	docker build --no-cache \
		-f Dockerfile-activator-simple \
...
test-invoke-backpressure:
	...
	PROXY_DATABASE_POOL_SIZE=50 \
	SPAWN_STATESTORE_KEY=3Jnb0hZiHIzHTOih7t2cTEPEpY98Tu1wvQkPfq/XwqE= \
	iex --name spawn@127.0.0.1 -S mix test test/invoke_backpressure_test.exs
...
run-benchmark:
	...
	PROXY_DATABASE_POOL_SIZE=50 \
	SPAWN_STATESTORE_KEY=3Jnb0hZiHIzHTOih7t2cTEPEpY98Tu1wvQkPfq/XwqE= \
	iex --name spawn@127.0.0.1 -S mix run benchmark.exs

run-proxy-local:
	ERL_ZFLAGS='-proto_dist inet_tls -ssl_dist_optfile rel/overlays/local-mtls.ssl.conf' \
	cd spawn_proxy/proxy && mix deps.get && \
	PROXY_DATABASE_TYPE=$(database) \
	PROXY_HTTP_PORT=9002 \
...
run-proxy-local2:
	...
	SPAWN_STATESTORE_KEY=3Jnb0hZiHIzHTOih7t2cTEPEpY98Tu1wvQkPfq/XwqE= \
	iex --name spawn_a2@test.default.svc -S mix
...
run-proxy-local-dicegame:
	...
	PROXY_HTTP_PORT=9001 \
	SPAWN_STATESTORE_KEY=3Jnb0hZiHIzHTOih7t2cTEPEpY98Tu1wvQkPfq/XwqE= \
	PROXY_ACTOR_SYSTEM_NAME=game-system \
	SPAWN_SUPERVISORS_STATE_HANDOFF_CONTROLLER=persistent \
	SPAWN_USE_INTERNAL_NATS=true \
	SPAWN_PUBSUB_ADAPTER=nats \
...
run-sdk-local:
	...
	PROXY_DATABASE_TYPE=$(database) \
	SPAWN_STATESTORE_KEY=3Jnb0hZiHIzHTOih7t2cTEPEpY98Tu1wvQkPfq/XwqE= \
	iex --name spawn_actors_node@127.0.0.1 -S mix

run-sdk-local2:
	cd spawn_sdk/spawn_sdk_example && mix deps.get && \
	PROXY_CLUSTER_STRATEGY=epmd \
	PROXY_DATABASE_POOL_SIZE=5 \
	SPAWN_USE_INTERNAL_NATS=true \
	SPAWN_PUBSUB_ADAPTER=nats \
	PROXY_DATABASE_TYPE=$(database) \
	SPAWN_STATESTORE_KEY=3Jnb0hZiHIzHTOih7t2cTEPEpY98Tu1wvQkPfq/XwqE= \
...
run-operator-local:
	cd spawn_operator/spawn_operator && mix deps.get && \
	MIX_ENV=dev \
	BONNY_POD_NAME=spawn-operator \
	iex --name operator@127.0.0.1 -S mix

run-activator-local:
	cd spawn_activators/activator && mix deps.get && \
	MIX_ENV=dev \
	iex --name activator@127.0.0.1 -S mix

run-activator-grpc-local:
	cd spawn_activators/activator_api && mix deps.get && \
	MIX_ENV=dev \
	iex --name activator_api@127.0.0.1 -S mix

run-activator-kafka-local:
	cd spawn_activators/activator_kafka && mix deps.get && \
	MIX_ENV=dev \
	iex --name activator_kafka@127.0.0.1 -S mix

run-activator-pubsub-local:
	cd spawn_activators/activator_pubsub && mix deps.get && \
	MIX_ENV=dev \
	iex --name activator_pubsub@127.0.0.1 -S mix

run-activator-rabbitmq-local:
	cd spawn_activators/activator_rabbitmq && mix deps.get && \
	MIX_ENV=dev \
	iex --name activator_rabbitmq@127.0.0.1 -S mix

run-activator-sqs-local:
	cd spawn_activators/activator_sqs && mix deps.get && \
	MIX_ENV=dev \
	iex --name activator_sqs@127.0.0.1 -S mix
