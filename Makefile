...
build-cli:
	cd spawnctl/ && MIX_ENV=prod mix local.rebar --force && MIX_ENV=prod mix local.hex --force && MIX_ENV=prod mix deps.get && MIX_ENV=prod mix release.init && MIX_ENV=prod mix release spawnctl
...
run-proxy-local-3:
	cd spawn_proxy/proxy && mix deps.get && SPAWN_PROXY_LOGGER_LEVEL=info PROXY_CLUSTER_STRATEGY=epmd SPAWN_USE_INTERNAL_NATS=false SPAWN_PUBSUB_ADAPTER=native PROXY_DATABASE_PORT=3307 PROXY_DATABASE_TYPE=mariadb PROXY_DATABASE_POOL_SIZE=30 PROXY_HTTP_PORT=9003 USER_FUNCTION_PORT=8091 SPAWN_STATESTORE_KEY=3Jnb0hZiHIzHTOih7t2cTEPEpY98Tu1wvQkPfq/XwqE= iex --name spawn_a3@127.0.0.1 -S mix
